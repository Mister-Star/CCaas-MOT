...
std::vector<uint64_t> client_listener_thread_ids;
...
void ClientListenerMain(void)
{
    sigjmp_buf local_sigjmp_buf;
    MemoryContext clientlistener_context;
    ...
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "client listener", MEMORY_CONTEXT_STORAGE);

    clientlistener_context = AllocSetContextCreate(t_thrd.top_mem_cxt,
        "client listener",
        ALLOCSET_DEFAULT_MINSIZE,
        ALLOCSET_DEFAULT_INITSIZE,
        ALLOCSET_DEFAULT_MAXSIZE);
    (void)MemoryContextSwitchTo(clientlistener_context);
    ...
    if (sigsetjmp(local_sigjmp_buf, 1) != 0) {
        ...
        (void)MemoryContextSwitchTo(clientlistener_context);
        FlushErrorState();
        MemoryContextResetAndDeleteChildren(clientlistener_context);
        RESUME_INTERRUPTS();
        pg_usleep(1000000L);
        smgrcloseall();
    }
    ...
    pgstat_report_appname("client listen");
    ...
    for(auto &i : client_listener_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
        ...
        if (t_thrd.clientlistener_cxt.got_SIGHUP) {
            t_thrd.clientlistener_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.clientlistener_cxt.shutdown_requested) {
            proc_exit(0);
        }
        FDWClientListenerThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.clientlistener_cxt.got_SIGHUP = true;
    if (t_thrd.proc)
...
{
    int save_errno = errno;
    t_thrd.clientlistener_cxt.shutdown_requested = true;
    if (t_thrd.proc)
        SetLatch(&t_thrd.proc->procLatch);
    errno = save_errno;
}
