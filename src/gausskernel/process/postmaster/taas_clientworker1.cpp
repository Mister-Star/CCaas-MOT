...
std::vector<uint64_t> client_worker1_thread_ids;
...
void ClientWorker1Main(void)
{
    ...
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "client worker1", MEMORY_CONTEXT_STORAGE);
    ...
    pgstat_report_appname("client worker1");
    ...
    for(auto &i : client_worker1_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
        ...
        if (t_thrd.clientworker1_cxt.got_SIGHUP) {
            t_thrd.clientworker1_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.clientworker1_cxt.shutdown_requested) {
            proc_exit(0);
        }
        FDWClientWorker1ThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.clientworker1_cxt.got_SIGHUP = true;
    if (t_thrd.proc)
...
{
    int save_errno = errno;
    t_thrd.clientworker1_cxt.shutdown_requested = true;
    if (t_thrd.proc)
        SetLatch(&t_thrd.proc->procLatch);
    errno = save_errno;
}
