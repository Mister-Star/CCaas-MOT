...
std::vector<uint64_t> storage_manager_thread_ids;
...
void StorageManagerMain(void)
{
    ...
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "storage manager", MEMORY_CONTEXT_STORAGE);
    ...
    pgstat_report_appname("storage manager");
    ...
    for(auto &i : storage_manager_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
        ...
        if (t_thrd.storagemanager_cxt.got_SIGHUP) {
            t_thrd.storagemanager_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.storagemanager_cxt.shutdown_requested) {
            proc_exit(0);
        }
        FDWStorageManagerThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.storagemanager_cxt.got_SIGHUP = true;
    if (t_thrd.proc)
...
{
    int save_errno = errno;
    t_thrd.storagemanager_cxt.shutdown_requested = true;
    if (t_thrd.proc)
        SetLatch(&t_thrd.proc->procLatch);
    errno = save_errno;
}
