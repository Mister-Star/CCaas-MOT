...
std::vector<uint64_t> storage_message_manager_thread_ids;
...
void StorageMessageManagerMain(void)
{
    ...
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "storage message manager", MEMORY_CONTEXT_STORAGE);

    context = AllocSetContextCreate(t_thrd.top_mem_cxt,
        "storage message manager",
    ...
    pgstat_report_appname("storage message manager");
    ...
    for(auto &i : storage_message_manager_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
        ...
        if (t_thrd.storagemessagemanager_cxt.got_SIGHUP) {
            t_thrd.storagemessagemanager_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.storagemessagemanager_cxt.shutdown_requested) {
            proc_exit(0);
        }
        FDWStorageMessageManagerThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.storagemessagemanager_cxt.got_SIGHUP = true;
    if (t_thrd.proc)
...
{
    int save_errno = errno;
    t_thrd.storagemessagemanager_cxt.shutdown_requested = true;
    if (t_thrd.proc)
        SetLatch(&t_thrd.proc->procLatch);
    errno = save_errno;
}
