...
std::vector<uint64_t> storage_sender_thread_ids;
...
void StorageSenderMain(void)
{
    ...
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "storage sender", MEMORY_CONTEXT_STORAGE);
    ...
    pgstat_report_appname("storage sender");
    ...
    for(auto &i : storage_sender_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
        ...
        if (t_thrd.storagesender_cxt.got_SIGHUP) {
            t_thrd.storagesender_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.storagesender_cxt.shutdown_requested) {
            proc_exit(0);
        }
        FDWStorageSenderThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.storagesender_cxt.got_SIGHUP = true;
    if (t_thrd.proc)
...
{
    int save_errno = errno;
    t_thrd.storagesender_cxt.shutdown_requested = true;
    if (t_thrd.proc)
        SetLatch(&t_thrd.proc->procLatch);
    errno = save_errno;
}
