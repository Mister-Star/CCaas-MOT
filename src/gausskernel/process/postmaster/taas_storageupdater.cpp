...
std::vector<uint64_t> storage_updater_thread_ids;
...
void StorageUpdaterMain(void)
{
    ...
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "storage updater", MEMORY_CONTEXT_STORAGE);
    ...
    pgstat_report_appname("storage updater");
    ...
    for(auto &i : storage_updater_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
        ...
        if (t_thrd.storageupdater_cxt.got_SIGHUP) {
            t_thrd.storageupdater_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.storageupdater_cxt.shutdown_requested) {
            proc_exit(0);
        }
        FDWStorageUpdaterThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.storageupdater_cxt.got_SIGHUP = true;
    if (t_thrd.proc)
...
{
    int save_errno = errno;
    t_thrd.storageupdater_cxt.shutdown_requested = true;
    if (t_thrd.proc)
        SetLatch(&t_thrd.proc->procLatch);
    errno = save_errno;
}
