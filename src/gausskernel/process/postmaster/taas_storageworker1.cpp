...
std::vector<uint64_t> storage_worker1_thread_ids;
...
void StorageWorker1Main(void)
{
    ...
    t_thrd.utils_cxt.CurrentResourceOwner = ResourceOwnerCreate(NULL, "storage worker1", MEMORY_CONTEXT_STORAGE);
    ...
    pgstat_report_appname("storage worker1");
    ...
    for(auto &i : storage_worker1_thread_ids ){
        if(i == thread_id){
            break;
        }
        id++;
    ...
    for (;;) {
        ...
        if (t_thrd.storageworker1_cxt.got_SIGHUP) {
            t_thrd.storageworker1_cxt.got_SIGHUP = false;
            ProcessConfigFile(PGC_SIGHUP);
        }

        if (t_thrd.storageworker1_cxt.shutdown_requested) {
            proc_exit(0);
        }
        FDWStorageWorker1ThreadMain(id);
...
static void _SigHupHandler(SIGNAL_ARGS)
{
    ...
    t_thrd.storageworker1_cxt.got_SIGHUP = true;
    if (t_thrd.proc)
...
{
    int save_errno = errno;
    t_thrd.storageworker1_cxt.shutdown_requested = true;
    if (t_thrd.proc)
        SetLatch(&t_thrd.proc->procLatch);
    errno = save_errno;
}
